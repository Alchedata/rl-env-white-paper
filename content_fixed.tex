...
\begin{figure}[tbp]
    ...
    \begin{tikzpicture}[
        ...
        tick/.style={font=\footnotesize\sffamily, text=gray!80},
        env/.style={circle, draw, minimum size=1.0cm, font=\tiny\sffamily, align=center, thick},
        quality/.style={ellipse, draw=green!60!black, fill=green!10, thick, minimum width=3.8cm, minimum height=2.4cm, font=\small\bfseries\sffamily, text=green!50!black, opacity=0.8}
    ]
    % Simple Gradient-like Circles for background context
    \fill[green!5, opacity=0.5] (3.5,3.0) circle (2.2cm);
    \fill[yellow!5, opacity=0.5] (1.0,1.0) circle (1.5cm);

    % Axes
    \draw[axis] (0,0) -- (6.5,0) node[label, right] {Diversity};
    \draw[axis] (0,0) -- (0,5.5) node[label, above] {Fidelity};
    \draw[axis] (0,0) -- (-3.5,-2.5) node[label, below left] {Verifiability};
    
    % Quality zone with a "glow" effect
    \node[quality] (qz) at (4.0,3.5) {Quality Zone};
    \node[font=\tiny\itshape, text=green!40!black] at (4.0,3.0) {(Target Production Area)};
    
    % Arrows to indicate direction of improvement
    \draw[->, >=stealth, green!70!black, ultra thick, opacity=0.3] (1,1) -- (3.5,3);

    % Environment examples with color coding
    % Color key: red (low), orange (mid), blue (specialized), green (high quality)
    \node[env, fill=red!10, draw=red!40] at (0.8,0.8) {Toy\\Sim};
    \node[env, fill=orange!10, draw=orange!40] at (2.2,2.0) {Web\\Arena};
    \node[env, fill=blue!10, draw=blue!40] at (2.8,4.5) {Isaac\\Sim};
    \node[env, fill=yellow!10, draw=yellow!60!black] at (3.8,1.5) {SWE\\Bench};
    \node[env, fill=green!10, draw=green!40] at (4.8,4.2) {AWM};
    \node[env, fill=cyan!10, draw=cyan!40] at (5.2,2.8) {Reasoning\\Gym};
    \node[env, fill=purple!10, draw=purple!40] at (0.5,4.0) {Static\\Dataset};
